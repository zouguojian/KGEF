...
class Experiment:
    ...
    def evaluate(self, model, data):
        ...
        for i in range(0, len(test_data_idxs), self.batch_size):
            data_batch, target = self.get_batch(er_vocab, test_data_idxs, i) #data_batch: [batch, 3]
            ...
            for j in range(data_batch.shape[0]):
                ...
                target_value = predictions[j,e2_idx[j]].item() #返回一个值
                predictions[j, filt] = 0.0
                predictions[j, e2_idx[j]] = target_value

            sort_values, sort_idxs = torch.sort(predictions, dim=1, descending=True) #将prediction tesor按照指定维度降序排序
    ...
    def train_and_eval(self):
        ...
        for it in range(1, self.num_iterations+1):
            ...
            for j in range(0, len(er_vocab_pairs), self.batch_size):
                data_batch, targets = self.get_batch(er_vocab, er_vocab_pairs, j) #得到输入值和真实的标签
                opt.zero_grad() #清楚梯度
                e1_idx = torch.tensor(data_batch[:,0])   #输入张量化
                r_idx = torch.tensor(data_batch[:,1])
                if self.cuda:
                    e1_idx = e1_idx.cuda()
                    r_idx = r_idx.cuda()
